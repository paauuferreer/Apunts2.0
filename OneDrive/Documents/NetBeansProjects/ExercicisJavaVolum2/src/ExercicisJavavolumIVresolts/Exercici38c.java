...
/**
 * 38. Genera sencers aleatòris entre un valor màxim i un valor mínim fins que els hagis generats tots.
...
void main() {
     ...
     */
    boolean[] generat = new boolean[MAXIM + 1];
    /**
    ...
    do {
        ...
        generat[aleatori] = true;
        comptador++;

        IO.println("Aleatori: " + aleatori);
        //Comprova si els ha generat tots
        complet = true;
//            for (int i = MINIM; i < generat.length; i++) {
//                complet = complet && generat[i];
//            }
        for (int i = MINIM; i < generat.length && complet; i++) {
            complet = generat[i];
        }
    } while (!complet);
